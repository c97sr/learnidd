...
## Format
  ...
  **Timing** We have 2 1/2 hours to meet our objectives. However, I would expect people to progress at quite different rates depending on their familiarity with the concepts and their experience with R. At various points in time we will come together as a group to review answers and to discuss issues that arise. We will break for 30 minutes after 90 minutes and we will use the last 10 minutes to address any outstanding issues.
...
For each farm, you have only two fields: **easting** and **northing**. The former is
the x location in metres and the latter is the y location. These 6-figure values
form the standard coordinate system for the UK: if you ever need to tell someone
exactly where you are, give them these all 12 digits, with easting first, as your
"grid reference".

## 1. Setting up the system for simulation

1. Open `R` and load the data set.
```{r, eval=TRUE}
library(learnidd)
path <- "/Volumes/kainslie/teaching/learnidd/"
source(paste0(path,"R/spatial_practical.R"))
```
2. Set up an answer document in Word and make sure you can copy and paste
charts from R to Word.
3. Prepare the simulation and plot easting by northing.
```{r, echo=TRUE}
farmdf <- adm.load.sheep.prep.sim(filename = paste0(path,"data/msc_farm_data.csv"))
...
**Q 1.2** Enter the following commands and describe in words the changes in the chart you made for the previous question. Note that you need to press the return key after the & sign on the second line. The R interpreter will know you are not finished and will add the + sign for you at the start of the next line.
```{r, eval=FALSE}
attach(farmdf)
...
detach(farmdf)
```
The function `head()` gives the first 5 rows of a data frame. Type the following at the R prompt:
```{r, echo = TRUE}
head(farmdf)
```
...
To obtain the source code for any function in R, you just type the name of the function without the brackets (). Also, you can open the file spatial.R in the R editor and examine the source.

**Q 1.4** Look at the source code for adm.seed.sheep and arm.apply.seed and describe what they do.
...
We will use the function `adm.sheep.params` to generate parameters of our model. Try the following commands at the `R` prompt:
```{r, eval=FALSE}
p1 <- adm.sheep.params()
p1
p2 <- adm.sheep.params()
...
p2
```
This is a generational model - we do not need to worry about time. The spatial transmission kernel k(d) in this model is defined as the probability that an infectious farm in generation n infects a susceptible farm at distance d in generation n + 1. The command `sapply(X,f)` applies the function f to each member of the vector X and returns a vector of the same length as X. Enter the following commands:
```{r, eval=FALSE}
xvals <- (1:150)*100
...
table(farmdf$g_i)
system.time(for (i in 1:2) farmdf[,"g_i"] <- adm.apply.gen.model(p1,farmdf))
```
...
**Q 2.4** Do you ovserve exponential growth?
...
**Q 4.1** Define a transmission kernel that causes only a small outbreak when seeded in the north east of the region but causes a large outbreak in the middle of the region.

**Q 4.2** By a process of trial and error (or otherwise) find values for the kernel
parameters that make the kernel almost flat.
...
## 5. Reactive vaccination
If time permits, the demonstrator will set a specific challenge for you, that you must complete without using your computer.

## 6. Further questions

**Q 6.1** Based on your lectures this morning, what other techniques are there for improving the efficiency of the simulation algorithm we have used today? How would you design the auxiliary data structures you need for such improvements? Design and implement a refinement of the function `adm.apply.gen.model` that is even faster.

**Q 6.2** The simple vaccination scenario we discussed in section 5 was not at all realistic. Search for "foot-and-mouth vaccination" on pubmed and read the abstracts of some modelling studies. What other types of vaccination strategy might you want to try? How would you design data structures and function to test your strategies? Implement a ring and a doughnut vaccination policy and compare them for different kernels.

**Q 6.3** Are there other aspects of this practical that you have found interesting?
Suggest an interesting question to the demonstrator.